...
describe("updatePlanProgram", () => {
  it("returns an error without calling fetch when there is no session token", async () => {
    ...
    const result = await updatePlanProgram("plan-1", program, EXPECTED_VERSION, undefined, {
      ...OPTIONS,
      fetchImpl,
    });
  ...
  it("PUTs the full program and the expected version to the plan's program path", async () => {
    ...
    await updatePlanProgram("plan-1", program, EXPECTED_VERSION, TOKEN, {
      ...OPTIONS,
      fetchImpl,
    });
  ...
  it("returns the saved program and its new version on a 200", async () => {
    ...
    const result = await updatePlanProgram("plan-1", program, EXPECTED_VERSION, TOKEN, {
      ...OPTIONS,
      fetchImpl,
    });
