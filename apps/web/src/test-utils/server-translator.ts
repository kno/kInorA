/**
 * `createServerTranslator` — builds a real EN-catalog `t()` function for
 ...
 * under the hood, seeded with the REAL production EN catalog, so a test
 * failure means the component genuinely renders the wrong catalog key.
 */
import { createTranslator } from "use-intl/core";
import { catalogs } from "@kinora/i18n";

export function createServerTranslator() {
  return createTranslator({ locale: "en", messages: catalogs.en });
}
