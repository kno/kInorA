...
| `OPENROUTER_MODEL` | Yes (prod) | OpenRouter model identifier in `provider/model` namespace. **Must** be a model that supports JSON-schema structured output (e.g. `openai/gpt-4o-mini`, `anthropic/claude-3-5-haiku`). Models without JSON-schema or function-calling support will fail at generation time. |
