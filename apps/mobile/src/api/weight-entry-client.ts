/**
 * Mobile bodyweight time-series API client (17c-profile-body-metrics, PR 5).
 *
 * Mirrors the web app's `weight-entry-client.ts`'s calling convention
 * exactly: an injectable `fetch` for unit testing, the same Bearer-token
 * call shape, and a discriminated result envelope — adapted to the mobile
 * runtime (Expo SecureStore token, direct API calls, no server action).
 *
 * Endpoints:
 *   GET  /weight-entries        → 200 { entries: WeightEntryDTO[] }
 *   POST /weight-entries {…}    → 201 { entry, wasFirstEntry } | 422 { error }
 */
 ...
 * UNCHANGED — the `(0, 500]` weight bound and the future-date rejection
 * live at the route only, never duplicated here.
 */
export async function createWeightEntry(
  ...
  return {
    ...
    wasFirstEntry: responseBody.wasFirstEntry,
  };
}
