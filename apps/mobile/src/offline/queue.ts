 ...
 * (Phase 5 mobile offline — design "Queue is FIFO with last-write-wins
 * collapse"). Mirrors web's `apps/web/src/app/(app)/plan/offline/queue.ts`
 * exactly; only the underlying storage adapter differs (AsyncStorage here,
 * idb there).
 *
 ...
 * `identity.ts`) so a shared-device login switch never mixes queues
 * between accounts.
 */
 ...
 * read-modify-write. AsyncStorage has no native transactions, so the
 * concrete adapter (`async-storage-db.ts`) must itself serialize concurrent
 * `incrementCounter` calls through an in-process async mutex; a naive
 * separate `getItem` + `setItem` pair would let two overlapping callers on
 * the SAME identity (e.g. a rapid double-tap dispatching two enqueues before
 * the first's await resolves) both read the same high-water-mark before
 * either writes, silently clobbering each other's mutation at the same
