...
(Previously: the system MUST compare completed sessions against planned sessions over a configurable period, with only a single "low adherence detected" scenario and no percentage formula, default window, purity/determinism constraint, or insufficient-data handling.)
...
(Previously: the system SHOULD recommend lower frequency or volume when adherence drops below 70%, with only the single 31%→4→3 scenario and no `suggestedChange` shape, floor rule, determinism constraint, or already-at-floor handling.)
...
(Previously: adherence-based changes MUST be presented as suggestions requiring user confirmation, with only the single "user rejects → plan unchanged" scenario and no accept/regenerate path, quota-on-confirm rule, auto-apply prohibition, or quota-exhausted handling.)
...
- **Window default (design decision)**: the fixed behavior is a configurable rolling window with default 4 weeks and a `< 70%` low threshold; the exact `toDays` reduction curve beyond `4 → 3` and the "sane minimum" floor value are left to design (default: reduce by one day/week, floor 1).
- **Read-surface shape (design decision)**: whether the recommendation is folded into `DashboardSummaryDTO` or exposed via a dedicated `GET /adaptation/adherence` is left to design; the fixed behavior is a tenant/user-scoped, on-demand, no-quota read.
- **Quota-exhausted status (design)**: the exact HTTP status/reason for a confirm-time quota-exhausted error is left to design; the fixed behavior is a clear error with the plan unchanged.
- **Mobile sequencing**: the mobile banner (Track D) depends on the RN plan/regenerate port (Track C); web (Tracks A+B) ships independently.
