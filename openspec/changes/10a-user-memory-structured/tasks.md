...
- [x] 3.2 [RED] Test: `deleteAllByUser` deletes all completed sessions, returns count, returns 0 when none, 409 when active exists, cascade removes exercises+sets
- [x] 3.2 [GREEN] Add `deleteAllByUser(tenantId, userId)` to `WorkoutSessionRepository` with active guard
...
- [x] 3.4 [RED] Test route: `DELETE /workout-sessions` — 200 + `{ deletedCount }`, 409 on active, 200 + `{ deletedCount: 0 }` when empty
- [x] 3.4 [GREEN] Add `DELETE /workout-sessions` to workout-session routes
...
- [x] 3.6 Modify `AuthService.getProfile` to return `name` from `userProfiles` with email-derived initials fallback
