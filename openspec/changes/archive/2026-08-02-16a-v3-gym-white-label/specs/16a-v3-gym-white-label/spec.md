# Delta for 16a-v3-gym-white-label
...
`subdomainSlug`.
(Previously: described only "logo, colors, and domain/subdomain branding" with
no concrete gating mechanism, palette shape, or validation.)
...
(Previously: stated the fallback outcome without the concrete `var()`
mechanism or the unknown-subdomain case.)
...
PII.
(Previously: described only "applies only to the owning gym tenant" without
naming the public endpoint's exposure limits.)
