...
- [x] 10.1.1 [RED] For each `trackerCopy` function entry, write test asserting the ICU message key renders output identical to the old function for representative inputs, in both ES and EN. `apps/mobile/src/copy/__tests__/tracker-migration.test.ts` — es assertions are byte-identical to the pre-migration `copy/tracker.ts` literals (baked in directly so the test survives 10.1.4's deletion); en assertions pin the new (previously nonexistent) English translation against the catalog.
- [x] 10.1.2 [GREEN] Migrate `apps/mobile/src/copy/tracker.ts` entries to ICU messages; screens switch to `useIntl().formatMessage({id}, args)` / `<FormattedMessage/>`. All 45 entries mapped: 22 reuse the existing web `tracker.*` namespace verbatim, 23 reuse the mobile-only `mobileTracker.*` namespace authored in slice 9 — zero new keys invented.
- [x] 10.1.3 [GREEN] Migrate `WorkoutTrackerScreen` and remaining tracker components referencing old `trackerCopy` constants to consume the runtime. `WorkoutTrackerScreen.tsx` now calls `useIntl()` and routes every old `trackerCopy.*` call-site through a local catalog-key router (same method names/signatures as the old API, zero hardcoded fallback text) — RED render test written first in `apps/mobile/src/screens/__tests__/WorkoutTrackerScreen.test.tsx` (4/4 failed pre-migration, confirming it exercised real behavior), then GREEN after migration.
- [x] 10.1.4 Delete `apps/mobile/src/copy/tracker.ts` once all call-sites are migrated — grep-verify zero remaining references. Deleted; grep for `trackerCopy|copy/tracker` across `apps/mobile/src` returns only doc-comment mentions in the migrated screen and its own historical test file, zero real imports/call-sites.

### Phase 10.2: `select` + Number/Date Parity (SC#3, mirrors web Gap 1)
- [x] 10.2.1 [RED] Write test asserting a mobile screen renders an ICU `{select}` branch (e.g. plan/session status equivalent) via `<FormattedMessage>`, matching the branch selection logic proven on web in slice 5. Reuses the EXISTING `plan.selector.option` key (slice 5, web) — no mobile-only key invented; mobile has no plan-selector screen in this slice's scope, so `apps/mobile/src/i18n/__tests__/select-number-date.test.tsx` exercises the runtime capability directly (same convention as the slice-9 cross-runtime-parity test), covering all 4 branches (ready/generating/failed/other) in both locales.
- [x] 10.2.2 [RED] Write test asserting `<FormattedDate>`/`<FormattedNumber>` render locale-correct date/number output for a shared catalog key with date/number args. This catalog deliberately keeps dates OUT of ICU strings (web formats dates via `useFormatter().dateTime` on a raw `Date`, never a `{date}` ICU arg — see `plan.selector.option`'s design note) — mirrored here with `<FormattedDate>`/`<FormattedNumber>` exercised directly on raw values, matching that same convention.
- [x] 10.2.3 [GREEN] Confirm both 10.2.1 and 10.2.2 pass with the migrated components from Phase 10.1. All pass; see the same test file's 10.2.3 describe block.

### Phase 10.3: Plural Parity with Web
- [x] 10.3.1 [RED] Write test asserting mobile plural selection (count=1 vs count=3) for a shared plural key matches the branch web selects for the same values. Extended `apps/mobile/src/i18n/__tests__/cross-runtime-parity.test.ts` with a second shared plural key (`tracker.timeline.meta.done`, beyond slice 9's `tracker.next.sets` proof) asserting BOTH the `one` and `other` branch with concrete rendered text, in both locales.
- [x] 10.3.2 [GREEN] Confirm parity holds under `react-intl`'s ICU engine (regression guard; should hold by construction since it shares the MessageFormat engine with next-intl). Confirmed — parity held on first run, no discrepancy.

### Phase 10.4: Hermes On-Device Validation (apply-time)
- [ ] 10.4.1 **[VALIDATE — apply-time, NOT VALIDATED]** Render an ES plural message (e.g. `tracker.next.sets`) on a real Hermes device/simulator (RN 0.79.5); confirm `Intl.PluralRules` selects the correct ES plural branch for count=1 and count=3. **NOT VALIDATED by this apply pass** — the executing agent is headless and cannot run a real Hermes device/simulator; a Vitest/Node run is NOT a Hermes validation (Node's `Intl` differs from Hermes's `Intl.PluralRules` implementation) and is not being represented as one here. This item stays open, pending an apply-time human/device task. **Deferred, tracked in issue #117.**
- [ ] 10.4.2 **[DECISION — gated by 10.4.1, OPEN]** If Hermes native output is correct for ES → no polyfill added (design default). If incorrect → enable `@formatjs/intl-pluralrules` + `@formatjs/intl-locale` (force import before `IntlProvider` mount), add the dependency, re-run 10.4.1 to confirm the fix. **Left OPEN, undecided** — the code currently ships the design's DEFAULT path (no polyfill added, no `@formatjs/intl-pluralrules`/`@formatjs/intl-locale` dependency); this is NOT a confirmed-correct decision, only the unvalidated default. Must be resolved by an apply-time human/device check before this is considered closed. **Deferred, tracked in issue #117.**

### Phase 10.5: Final Verification
- [x] 10.5.1 `pnpm test` in `apps/mobile` — full suite green, including EN support parity across the full tracker surface (not just the slice-9 proof screen). 105/105 tests, 15/15 files green; also re-verified with `packages/{i18n,contracts,domain}/dist` renamed aside (no-dist CI-real check) — still 105/105, then restored; `pnpm type-check` clean in both states.
- [x] 10.5.2 Grep-verify zero remaining references to old Spanish-only hardcoded tracker copy anywhere in `apps/mobile/src`. Grep clean outside test files/doc comments (`Sesión activa`, `Pausar sesión`, `Terminala antes`, `Cargando sesión`, `Sesión completada`, `Reintentar`, etc. — zero real code matches).
