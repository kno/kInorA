...
(Previously: accepted natural-language input to produce a `PlanSpec` with only full-conversation and ambiguous-clarification scenarios; no field allow-list, enum/bounds validation, or fail-closed rule.)
...
(Previously: presented the extracted `PlanSpec` for review/edit with a single confirmation scenario; no shared-draft persistence or generation-path constraint.)
...
- **Denial status (design decision)**: the exact HTTP status for the Pro-gate denial (402 vs 403) is left to design; the fixed behavior is fail-closed with a `premium_required`-style reason before any LLM work.
- **Slice split (design)**: whether Slice 2 splits into S2a (transport + gate + fail-closed lifecycle) and S2b (structured extraction terminal event + masking) is a design decision; the specified events and boundaries are fixed.
