...
- [x] 3.1 RED: Extend `apps/api/src/routes/__tests__/plan-chat.test.ts` (or new `extraction-adapter.test.ts`): valid message → prose deltas then terminal `draft` event with `{draftSpec, missingFields, assistantMessage}`; injected Pass-2 (extraction) failure (Threat Matrix: mid-stream failure) → terminal `error` event, draft in `plan_drafts` left byte-for-byte unchanged; empty/whitespace input → no LLM call, clarifying prompt only.
- [x] 3.2 RED: Add failing masking assertion (Threat Matrix: health/limitation leak) — a turn containing limitation text produces a prompt/observability payload with the sensitive text masked via `mask()`/`sanitizeMemoryContext`, verified against the LangChain/Langfuse call args in the test double.
- [x] 3.3 RED: Add failing no-embedding assertion (Threat Matrix: raw-transcript embedding) — a completed chat turn triggers no vector-store write.
- [x] 3.4 GREEN: Create `apps/api/src/ai/extraction-adapter.ts` — Pass 1 `streamReply()` via LangChain `.stream()`; Pass 2 `extract()` via `withStructuredOutput(PlanSpecDraftSchema, {method:"jsonSchema"})`; applies masking from S1's `extraction-prompt.ts` before any LLM/Langfuse call.
- [x] 3.5 GREEN: Complete the `POST /plan-specs/chat` handler in `apps/api/src/routes/plan.ts` — after Pass 1 completes, run Pass 2, call `mergePlanSpecDraft` (S1), `repo.upsertDraft` (commit only here), then emit terminal `draft`; any Pass-1/Pass-2 error emits terminal `error` with the draft untouched. Confirm no `plan_generation` consumption anywhere in this path.
- [x] 3.6 GREEN: Swap `apps/api/src/app.ts` wiring from the S2a stub extractor to the real `extraction-adapter.ts` (Mock stays available for tests).
- [x] 3.7 TRIANGLE: Run the full `apps/api` chat suite green; confirm draft is committed exactly once and only on the terminal event (assert `upsertDraft` call count/timing in tests); confirm `deps-guard`-relevant imports (`langchain`/provider SDK) stay inside `apps/api/src/ai/`.
- [x] 3.8 REVIEW FIX (HIGH): `PlanSpecExtractor.extract` now accepts an optional `AbortSignal`, threaded from the route's shared `AbortController` into `PlanSpecExtractionAdapter.extract`'s `withStructuredOutput(...).invoke(input, { signal })` call, so a wall-clock timeout or disconnect firing DURING Pass 2 actually cancels the in-flight structured-output request instead of blocking until it resolves. RED test (`extraction-adapter.test.ts`: stalled Pass-2 call only settles on abort) confirmed failing before the fix, green after.
- [x] 3.9 REVIEW FIX (HIGH): `writeFrame`'s backpressure drain-wait no longer hangs — it bails immediately when `controller.signal` is already aborted before the wait starts, and otherwise races `'drain'` against `'close'`/`'error'` on `raw` and `abort`, removing all listeners on settle. RED test (`plan-chat.test.ts`: every `write()` reports backpressure and 'drain' never naturally fires) confirmed the handler previously would not reach `finally`/`raw.end()` within the race window; green after the fix.
- [x] 3.10 REVIEW FIX (WARNING): the route now computes `missingFields` from the current draft (`mergePlanSpecDraft(currentDraft, {})`) and includes it on `ChatExtractInput` for both `streamReply` and `extract`, restoring the clarifying-question steering `buildExtractionPrompt` depends on. Test asserts the captured input on an empty draft lists all six missing fields.
- [x] 3.11 REVIEW FIX (SUGGESTION): corrected the misleading `app.ts` chat-wiring comment — production always uses the real `PlanSpecExtractionAdapter` (no silent Mock fallback); an unconfigured provider key fails at call time with the provider's own error, not a stubbed Mock.
- [x] 3.12 Added a code comment at the two-pass site (`extraction-adapter.ts` class doc) acknowledging Pass 1 (prose) and Pass 2 (extraction) are independent LLM calls that MAY disagree; the committed draft is always Pass 2's output. No behavior change. Left `TODO(S3)` in `plan.ts` documenting the known concurrent-overlapping-turns lost-update (mitigated client-side in S3, not fixed here).
...
- [x] 4.1 RED: Add failing component test for create-plan server component: Pro effective tier (reusing 11b billing-visibility fetch) → default mode Asistente; Free → default mode Formulario + Asistente teaser/"Mejora a Pro" CTA (reusing `#pro-card`) rendered; Free cannot trigger a chat turn.
- [x] 4.2 GREEN: Modify `apps/web/.../create-plan/page.tsx` to derive effective tier server-side (reuse existing billing-visibility loader) and pass default mode + teaser flag to the client component.
- [x] 4.3 RED: Add failing SSE-consumer test (mocked `ReadableStream`) for incremental token render, terminal `draft` event populating the "Datos extraídos" panel, mid-stream error showing a retry affordance without losing prior draft state, and `AbortController.abort()` firing on component unmount (deps-guard: no `langchain|openai|langfuse|ai-sdk` import in this module).
- [x] 4.4 GREEN: Create the Asistente chat pane consuming `POST /plan-specs/chat` via `fetch` + `ReadableStream` (not `EventSource` — needs POST + `Authorization` header), rendering incremental assistant text, updating local state from the terminal `draft` event, and aborting on unmount/navigation.
- [x] 4.5 GREEN: Create the "Datos extraídos" per-field review/edit panel (populate from terminal event, edits persist to the shared draft via existing draft-update calls) and the `btn-asistente`/`btn-formulario` mode toggle wired to the existing draft/promote/confirm actions — no new generation entry point.
- [x] 4.6 GREEN: Add the `chat` i18n namespace to `packages/i18n/src/messages/{en,es}.json` (assistant copy, clarifying-question strings, teaser/CTA copy) with EN/ES key parity.
...
- [x] 4.8 REVIEW FIX (WARNING): fixed the error-path thread glitch in `AssistantPane.tsx` — `runTurn` now takes an `appendUserMessage` flag; a terminal error/exception removes the trailing assistant placeholder ONLY if it is still empty (`removeTrailingEmptyAssistant`), so no blank coach bubble is ever rendered; `handleRetry` calls `runTurn(lastUserMessageRef.current, false)`, resending the same turn WITHOUT appending another user bubble. RED test (`AssistantPane.test.tsx`: send → terminal error → assert no empty `[data-role="assistant"]` bubble + retry affordance shown; click retry → assert the user message is NOT duplicated + fetch called exactly twice + a normal assistant reply renders) confirmed failing before the fix, green after.
- [x] 4.9 REVIEW FIX (SUGGESTION): `isSpecComplete` in `AssistantPane.tsx` now also validates field RANGES via `PlanSpecDraftSchema.safeParse(spec)` (the same contract the server confirm gate uses) in addition to the existing presence check, so an out-of-range panel edit (daysPerWeek=0, sessionDurationMinutes=10) keeps "Generar plan" disabled instead of only failing server-side. Server confirm remains the real enforcement — unchanged. RED tests added: daysPerWeek=0 and sessionDurationMinutes=10 with all other fields present → Generate stays disabled; confirmed failing before the fix, green after.
